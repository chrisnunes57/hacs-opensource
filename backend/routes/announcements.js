...
router.post("/create", (request, response, next) => {
  ...
  Announcements.create(title, content)
    .then(() => {
      response
        .status(201)
        .json({ msg: `Announcement "${title}" has been saved.` });
    })
    .catch(error => {
...
router.post("/remove/:id", (request, response, next) => {
  ...
  Announcements.removeById(request.params.id)
    .then(() => {
      response
        .status(200)
        .json({ msg: `Removed Announcement ID: ${request.params.id}` });
    })
    .catch(error => {
      console.log(error);
      response.status(400).json({
        error: `Unable to remove Announcement ID: ${request.params.id}`
      });
    });
});
